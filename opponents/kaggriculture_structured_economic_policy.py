...
#
# This file is a FORK of a public Kaggle notebook agent, redistributed under the
# Apache License 2.0 with the original author's attribution preserved:
#
#   Original work : "238/238 Known Streams | v58 Minimax Closed Loop"
#   Author        : Kaito Fukami (Kaggle user kaitofukami)
#   Source        : https://www.kaggle.com/code/kaitofukami/238-238-known-streams-v58-minimax-closed-loop
#                   Version 11 (notebook page states: released under the Apache 2.0 open source license)
#   Upstream file : main.py decoded from the notebook's payload cell,
#                   sha256 b041058ec187a8d0a01edc0eab8de068b53deca3e6c1973faf74ace6916ddcb9
#   Pulled        : 2026-09-02 by Pilkwang Kim
#
# Changes by the fork author (2026-09-02), per Apache-2.0 section 4(b):
#   1. this header;
#   2. a loader shim appended at the END of the file that rebinds the module-level
#      policy to the name `agent` (the upstream file's last callable is
#      `kaggle_agent_v58`, a wrapper that calls `agent` by name; the fork's local
#      loader contract requires the last callable to be `agent`). The shim delegates
#      to the upstream policy without modification. Behaviour is unchanged.
# No other line of the upstream file has been modified.
#
# Provenance chain (fork author's records): upstream notebook -> payload decode
# (sha above) -> this file -> notebook build -> local execute -> Kaggle output,
# with sha256 equality asserted at each link.
...
# Loader shim (fork author, 2026-09-02): upstream binds its policy to the
# module-level name `agent` and then defines `kaggle_agent_v58` as a wrapper that
# calls `agent` by name. A raw-file loader selects the LAST callable, and the
# fork's loader contract requires that callable to be `agent`. Capture the policy
# under a private name, delete the binding, and rebind `agent` as the last
# callable. kaggle_agent_v58 -> agent -> _v58_policy; behaviour unchanged.
_v58_policy = agent
del agent


def agent(obs, configuration=None):
    return _v58_policy(obs, configuration)
