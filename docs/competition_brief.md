...
**警告:** moon-198 の 2543.4(2026-08-03)を「この盤面で 165 位=銀圏」と読んではならない。
**レーティングは相対値であり、異なる時点の値を同一尺度で比較することはできない。**
同一バイト列の再測定は 13 エピソードで 967.0、同格帯で 2勝4敗——**2543.4 は再現していない**
(`results/baseline/rating_convergence.md`)。2543.4 はコンペ開始4日目の値である。
...
- **moon-198 を再提出済み**(submission `55971967`、`main.py` sha256 `7c5a4a2a…`)。
  受理時 600.0 → 13エピソードで 967.0。**2543.4 は再現していない**(§2 の警告、および
  `results/baseline/rating_convergence.md`)。**現在の実力は 900〜1100級と推定され、
  銅ライン 1957.2 までは 2,500チーム分ある。ERL での上積みが必須である。**
